...
import { useScanValuesForDisconnectFromZP } from "../domain/useScanValuesForDisconnectFromZP";
...
import { useSendDisconnectFromZpData } from "../domain/useSendDisconnectFromZpData";
import TrayInfoForDisconnectFromZp from "./TrayInfoForDisconnectFromZp";
import GiveReasonModal from "./GiveReasonModal";
import { DeleteReason } from "@/features/shared/types/interfaces-disconnect_from_zp";
...
const DisconnectFromZpScanner = (props: Props) => {
  ...
  } = useScanValuesForDisconnectFromZP(setIsLoading);
  ...
  return (
    <View className="relative w-full h-full">
        ...
        <View className="w-full px-6 mt-4">
          <AppPath
            paths={[
              ...
              FIELD_CROPS,
              TRAY_OPERATIONS,
              { actionFn: () => {}, name: "Odepnij do bufora" },
            ]}
          />
        </View>
                ...
                <ScrollView className="w-full">
                  <View className="flex-row flex-wrap items-center justify-start py-4">
                    {scannedValues.map((tray, index) => (
                      <TrayInfoForDisconnectFromZp
                        key={tray.stk_id}
                        tray={tray}
                        deleteAction={showDeleteModal}
                        // changeDataFn={() => setIsGiveReasonModal(true)}
                        index={index}
                        setChosenTray={setChosenTray}
                      />
                    ))}
                  </View>
                </ScrollView>
      ...
      {/* delete Zp from list -  modal */}
      {/* <ModalInternal
        isOpen={isShowGiveReasonModal}
        isTransparent={false}
        backgroundColor={yellowColor}
      >
        <GiveReasonModal
          closeFn={() => setIsGiveReasonModal(false)}
          deleteReasons={deleteReasons}
          refreshAllData={refreshAllData}
          chosenTray={chosenTray}
          addOrChangeDeleteReason={addOrChangeDeleteReason}
        />
      </ModalInternal> */}
    </View>
  );
};
export default DisconnectFromZpScanner;
