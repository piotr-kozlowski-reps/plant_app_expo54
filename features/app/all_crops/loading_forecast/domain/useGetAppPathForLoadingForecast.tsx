...
export const useGetAppPathForLoadingForecast = () => {
  const getAppPathForLoadingForecast = (
    submoduleName: string,
    whatOrderType: AllLoadingForecastSubmodules
  ) => {
  ...
  function getBasePath(submoduleName: AllLoadingForecastSubmodules) {
    if (submoduleName === "greenhouse_crops_works_loading_forecast")
      return [INDEX, GREENHOUSE_CROPS, GREENHOUSE_CROPS_WORKS];
    return [INDEX, FIELD_CROPS, FIELD_CROPS_WORKS];
  }

  ////hook return
  return { getAppPathForLoadingForecast };
};
