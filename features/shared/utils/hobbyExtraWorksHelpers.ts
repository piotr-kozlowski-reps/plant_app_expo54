...
export const getIsHobbyExtraTechWorkPackingWithTj12 = (
  value: ExtraWork | number,
): boolean => {
  const activityId = 773247;
  if (typeof value === "number") return value === activityId;
  return value.keyval === activityId;
};
export const getIsHobbyExtraTechWorkPackingWithTj10 = (
  value: ExtraWork | number,
): boolean => {
  const activityId = 30667238;
  if (typeof value === "number") return value === activityId;
  return value.keyval === activityId;
};
